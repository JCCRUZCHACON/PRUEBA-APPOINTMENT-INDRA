import { saveAppointment, getAppointmentsByInsured } from "../functions/appointment/dynamoService";
...
describe("dynamoService", () => {
  test("saveAppointment should call dynamo put and return item", async () => {
    const data = {
  insuredId: "123",
  countryISO: "PE" as "PE", // 👈 fuerza el tipo literal
  schedule: {
    scheduleId: 1,
    centerId: 1,
    specialtyId: 2,
  ...
  test("getAppointmentsByInsured should call dynamo query", async () => {
    const insuredId = "123";
    const result = await getAppointmentsByInsured(insuredId);
